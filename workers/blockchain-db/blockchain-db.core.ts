...
import { PromiseOut, PromiseType } from '../../src/bnqkl-framework/PromiseExtends';
...
export class BlockchainDBCore extends EventEmitter {
  ...
  async getByHeightRange(gte: number, lte: number) {
    const pageDataCache = new Map<number, PromiseType<ReturnType<typeof BlockchainDBCore.prototype._getByPageFromCache>>>()
   ...
   * @param height
   */
  private async _getPageDataWithAutoCache(height: number, cache: Map<number, PromiseType<ReturnType<typeof BlockchainDBCore.prototype._getByPageFromCache>>>) {
    const page = (height / this._dbconfig.page_size) | 0;
    let pageData = cache.get(page);
    if (!pageData) {
      pageData = await this._getByPageFromCache(page);
