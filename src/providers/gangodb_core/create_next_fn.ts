...
import { build, Conjunction, Disjunction, Exists } from "./lang/filter";
...
const buildPredicates = pipeline => {
    const new_pipeline: Array<[any, any]> = [];
...
const addPipelineStages = ({ pipeline }: { pipeline: any[] }, next) => {
    for (var [fn, arg] of pipeline) {
        next = fn(next, arg);
    }
