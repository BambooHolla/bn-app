  ...
  async getRoundRemainTime () {
    ...
    if(roundTimeData.success) {
      ...
      if(blockTimeRes.success) {
        ...
        roundProgress = ((1-roundTime/(57*blockTime/1000))*100).toFixed(2);
  ...
  async vote(secret: string, secondSecret?: string) {
    try {
      ...
      if(voteTimestamp.success) {
        ...
        //获取投票的人
        await this.fetch.get<any>(vote_url)
          .then(async (resp) => {
            if(resp.success) {
              ...
              if(resp.delegate.length === 0) {
                ...
                let isDone:boolean = await this.transactionService.putTransaction(txData);
