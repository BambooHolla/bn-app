...
export class BlockChainDownloader extends EventEmitter {
   ...
   */
  async downloadBlocks(startHeight: number, endHeight: number, ownEndHeight: number, total?: number, asc?: boolean) {
    ...
    try {
      if (asc) {
        await this._download_with_auto_retry_asc(startHeight, endHeight, ownEndHeight, total);
      } else {
        await this._download_with_auto_retry(startHeight, endHeight, ownEndHeight, total);
      }
    } finally {
  ...
  private async _download_with_auto_retry(startHeight: number, endHeight: number, ownEndHeight: number, total?: number) {
    ...
    do {
      ...
      try {
        await this._download_range_blocks(pageSize, acc_endHeight, startHeight, ownEndHeight, total);
  ...
  private async _download_range_blocks(pageSize: number, acc_endHeight: number, startHeight: number, ownEndHeight: number, total: number) {
    ...
    const blocks: BlockModel[] = [];
    {
      ...
      for (var _b of list) {
        ...
        const block = {
          ...
          generatorPublicKey,
          generatorId: this.ifmJs.address().generateAddress(generatorPublicKey),
  ...
  // 升序下载
  private async _download_with_auto_retry_asc(startHeight: number, endHeight: number, ownEndHeight: number, total?: number) {
    if (typeof total !== "number") {
      total = ownEndHeight - startHeight + 1;
    }
    const pageSize = 10;
    ...
    do {
      ...
      let retry_interval = 1000;
      try {
        await this._download_range_blocks_asc(pageSize, acc_startHeight, endHeight, ownEndHeight, total);
  ...
  private async _download_range_blocks_asc(pageSize: number, acc_startHeight: number, endHeight: number, ownEndHeight: number, total: number) {
  ...
  async syncFullBlockchain(/*range_list:Range[],*/ max_end_height: number) {
    ...
    for (var _range of rangeHelper.ranges) {
      const range = _range;
      await this.downloadBlocks(range.start, range.end, max_end_height, max_end_height, true);
    }
    this.emit("end-sync", {});
    /** TODO:
