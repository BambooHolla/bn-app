    ...
    },
    {
      ...
      value: function verify(trs, sender, cb) {
        ...
        if (
          ...
          trs.asset.multisignature.lifetime > 72
        ) {
          // return setImmediate(cb, {
        ...
        if (this.ready(trs, sender)) {
          try {
            for (
              var s = 0;
              s < trs.asset.multisignature.keysgroup.length;
              s++
            ) {
              var verify = false;
              if (trs.signatures) {
        ...
        if (
          trs.asset.multisignature.keysgroup.indexOf("+" + sender.publicKey) !=
          -1
        ) {
          // return setImmediate(cb, {
        ...
        async.eachSeries(
          ...
          function(err) {
            ...
            var keysgroup = trs.asset.multisignature.keysgroup.reduce(function(
              p,
              c
            ) {
    ...
    },
    {
      ...
      value: function getBytes(trs, skip) {
        var keysgroupBuffer = new Buffer(
          trs.asset.multisignature.keysgroup.join(""),
          "utf8"
        );
    ...
    },
    {
      ...
      value: function ready(trs, sender) {
        ...
        if (!sender.multisignatures.length) {
          return (
            trs.signatures.length == trs.asset.multisignature.keysgroup.length
          );
        } else {
