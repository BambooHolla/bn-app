...
var IssueAssets = require("./issueAssets");
var DestoryAssets = require("./destoryAssets");
var TransferAssets = require("./transferAssets");
...
var Transaction = (function() {
    function Transaction() {
        ...
        this.attachAssetType(
            transactionTypes.DESTORY_ASSET,
            new DestoryAssets(),
        );
        this.attachAssetType(
    ...
    _createClass(Transaction, [
        {
            ...
            value: function attachAssetType(typeId, instance) {
                if (
                    ...
                    typeof instance.validateInput == "function"
                    // && typeof instance.verify == 'function'
                ) {
        ...
        },
        {
            ...
            value: function getBytes(trs, skipSignature, skipSecondSignature) {
                ...
                var bb;
                var assetBytes = privated.types[trs.type].getBytes.call(
                    this,
                    trs,
                    skipSignature,
                    skipSecondSignature,
                ...
                bb = new ByteBuffer(
                    1 + 4 + 32 + 32 + 64 + 20 + 64 + 64 + 20 + assetSize,
                    true,
                );
                bb.writeByte(trs.type);
                bb.writeInt(trs.timestamp);

                var senderPublicKeyBuffer = new Buffer(
                    trs.senderPublicKey,
                    "hex",
                );
                ...
                if (trs.requesterPublicKey) {
                    var requesterPublicKey = new Buffer(
                ...
                if (!skipSecondSignature && trs.signSignature) {
                    var signSignatureBuffer = new Buffer(
                        trs.signSignature,
                        "hex",
                    );
        ...
        },
        {
            ...
            value: function verify(trs, sender, requester, cb) {
                ...
                if (trs.requesterPublicKey) {
                    if (
                        sender.multisignatures.indexOf(trs.requesterPublicKey) <
                        0
                    ) {
                        return setImmediate(cb, {
                ...
                var multisignatures =
                    sender.multisignatures || sender.u_multisignatures;
                ...
                if (multisignatures.length == 0) {
                    if (
                        ...
                        multisignatures = trs.asset.multisignature.keysgroup.map(
                            function(key) {
                                return key.slice(1);
                            },
                        );
                ...
                if (trs.signatures) {
                    for (var d = 0; d < trs.signatures.length; d++) {
                        ...
                        for (var s = 0; s < multisignatures.length; s++) {
                            if (
                                trs.requesterPublicKey &&
                                multisignatures[s] == trs.requesterPublicKey
                            ) {
                                continue;
                            }
                ...
                if (
                    trs.amount < 0 ||
                    trs.amount > 100000000 * constants.fixedPoint ||
