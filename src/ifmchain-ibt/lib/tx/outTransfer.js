    ...
    _createClass(OutTransfer, [
        ...
        },
        {
            ...
            value: function process(trs, sender, cb) {
                library.dbLite.query(
                    "SELECT count(*) FROM dapps WHERE transactionId=$id",
                    {
                    ...
                    function(err, rows) {
                        ...
                        library.dbLite.query(
                            "SELECT count(*) FROM outtransfer WHERE outTransactionId = $transactionId",
                            {
                                transactionId:
                                    trs.asset.outTransfer.transactionId,
                            },
        ...
        },
        {
            ...
            value: function getBytes(trs) {
                var buf = void 0;
                buf = new Buffer([]);
                var dappIdBuf = new Buffer(
                    trs.asset.outTransfer.dappId,
                    "utf8",
                );
                var transactionIdBuff = new Buffer(
                    trs.asset.outTransfer.transactionId,
                    "utf8",
                );
        ...
        },
        {
            ...
            value: function apply(trs, block, sender, cb) {
                ...
                mod.getModules().accounts.setAccountAndGet(
        ...
        },
        {
            ...
            value: function objectNormalize(trs) {
                ...
                if (!report) {
                    throw Error(
                        "Can't verify dapp out transaction, incorrect parameters: " +
        ...
        },
        {
            ...
            value: function dbSave(trs, cb) {
                library.dbLite.query(
                    "INSERT INTO outtransfer(dappId, transactionId, outTransactionId) VALUES($dappId, $transactionId, $outTransactionId)",
                    {
