  ...
  _createClass(Vote, [
    ...
    },
    {
      ...
      value: function verify(trs, sender, cb) {
        ...
        if (
          trs.asset.votes &&
          trs.asset.votes.length > constants.maxVoteCount
        ) {
          return cb({
            message:
              "Voting limited exceeded. Maxmium is " +
              constants.maxVoteCount +
              " per transaction",
          });
        }
        cb(null, trs);
    ...
    },
    {
      ...
      value: function getBytes(trs) {
        var buf = void 0;
        buf = trs.asset.votes
          ? new Buffer(trs.asset.votes.join(""), "utf8")
          : null;
    ...
    },
    {
      ...
      value: function objectNormalize(trs) {
        ...
        if (!report) {
          throw new Error(
            "Incorrect votes in transactions: " + library.scheme.getLastError()
          );
        }
