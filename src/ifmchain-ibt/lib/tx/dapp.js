    ...
    _createClass(DApp, [
        ...
        },
        {
            ...
            value: function verify(trs, sender, cb) {
                ...
                //判断公钥是否合法
                if (
                    trs.asset.dapp.creatorPublicKey.length != 64 ||
                    !new Buffer(trs.asset.dapp.creatorPublicKey, "hex").length
                ) {
                    return cb({
                ...
                if (trs.asset.dapp.downloadAddress.split(";").length < 1) {
                    return cb({
                        message:
                            "Please fill in at least one type of installation package address",
                    });
                }
                ...
                if (!trs.asset.dapp.certificate) {
                    return cb({
                        message:
                            "You must upload your dapp's digital certificate",
                    });
                }
                ...
                if (
                    !trs.asset.dapp.name ||
                    trs.asset.dapp.name.trim().length == 0 ||
                    trs.asset.dapp.name.trim() != trs.asset.dapp.name
                ) {
                ...
                if (trs.asset.dapp.name.length > 32) {
                    ...
                    return cb({
                        message:
                            "Dapp name is too long. Maximum is 32 characters",
                    });
                }

                if (
                    trs.asset.dapp.description &&
                    trs.asset.dapp.description.length > 160
                ) {
                    // return setImmediate(cb, "Dapp description is too long. Maximum is 160 characters");
                    return cb({
                        message:
                            "Dapp description is too long. Maximum is 160 characters",
                    });
                }
            ...
            value: function getBytes(trs) {
                ...
                if (trs.asset.dapp.certificate) {
                    buf = Buffer.concat([
                        buf,
                        new Buffer(
                            trs.asset.dapp.certificate.toString(),
                            "utf8",
                        ),
                    ]);
                }
                ...
                if (trs.asset.dapp.icon) {
                    buf = Buffer.concat([
                        buf,
                        new Buffer(trs.asset.dapp.icon.toString(), "utf8"),
                    ]);
                }
                ...
                if (trs.asset.dapp.coverPicture) {
                    buf = Buffer.concat([
                        buf,
                        new Buffer(
                            trs.asset.dapp.coverPicture.toString(),
                            "utf8",
                        ),
                    ]);
                }
                ...
                if (trs.asset.dapp.downloadAddress) {
                    buf = Buffer.concat([
                        buf,
                        new Buffer(
                            trs.asset.dapp.downloadAddress.toString(),
                            "utf8",
                        ),
                    ]);
                }
                ...
                if (trs.asset.dapp.developerAddress) {
                    buf = Buffer.concat([
                        buf,
                        new Buffer(trs.asset.dapp.developerAddress, "utf8"),
                    ]);
                }
                ...
                if (trs.asset.dapp.creatorPublicKey) {
                    buf = Buffer.concat([
                        buf,
                        new Buffer(trs.asset.dapp.creatorPublicKey, "utf8"),
                    ]);
                }
                ...
                buf = Buffer.concat([
                    buf,
                    Buffer.from(bb.toString("hex"), "hex"),
                ]);
        ...
        },
        {
            ...
            value: function objectNormalize(trs) {
                for (var i in trs.asset.dapp) {
                    if (
                        trs.asset.dapp[i] === null ||
                        typeof trs.asset.dapp[i] === "undefined"
                    ) {
