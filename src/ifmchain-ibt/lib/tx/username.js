    ...
    _createClass(Username, [
        ...
        },
        {
            ...
            value: function verify(trs, sender, cb) {
                ...
                if (
                    !allowSymbols.test(trs.asset.username.alias.toLowerCase())
                ) {
                    // return setImmediate(cb, "User names can only be 1～20 digits, letters, underscores, and Chinese");
                    return cb({
                        message:
                            "User names can only be 1～20 digits, letters, underscores, and Chinese",
                    });
                }
                ...
                if (
                    trs.asset.username.alias.length === 0 ||
                    trs.asset.username.alias.length > 20
                ) {
                    // return setImmediate(cb, {
                    ...
                    return cb({
                        message:
                            "Invalid username length. Must be between 1 to 20 characters",
                    });
                }
