    ...
    _createClass(IssueAssets, [
        ...
        },
        {
            ...
            value: function verify(trs, sender, cb) {
                ...
                if (!trs.asset.issueAsset.originalIssuedAssets) {
                    return cb({
                        message:
                            "Assets original issued assets number is required",
                    });
                }
                ...
                if (!trs.asset.issueAsset.expectedRaisedIBTs) {
                    return cb({
                        message:
                            "Assets expected raised ibt number is required",
                    });
                }
                ...
                if (!trs.asset.issueAsset.expectedIssuedBlockHeight) {
                    return cb({
                        message:
                            "Assets expected issued block height is required",
                    });
                }
        ...
        },
        {
            ...
            value: function getBytes(trs) {
                ...
                var oiaBuf = Buffer.from(
                    trs.asset.issueAsset.originalIssuedAssets,
                );
                buf = Buffer.concat([buf, oiaBuf]);

                var ribtBuf = Buffer.from(
                    trs.asset.issueAsset.expectedRaisedIBTs,
                );
