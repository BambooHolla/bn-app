...
function createTransaction(body, cb) {
  tx.validateInput(body, function(err) {
    if (err) {
      var errorMessage =
        err[0].path.replace(/#\//g, "") + ": " + err[0].message;
    ...
    if (body.multisigAccountPublicKey) {
      ...
      data.senderId = addressHelper.generateBase58CheckAddress(
        keypair.senderPublicKey.toString("hex")
      );
    ...
    var trs = {
      ...
      requesterPublicKey: data.requester
        ? data.requester.publicKey.toString("hex")
        : null,
