...
module.exports.connect = function(connectString, cb) {
  ...
  var sql = [
    ...
    "CREATE TABLE IF NOT EXISTS fabulous(address VARCHAR(21) NOT NULL, transactionId VARCHAR(20) NOT NULL, FOREIGN KEY(transactionId) REFERENCES trs(id) ON DELETE CASCADE)",
    ...
    "CREATE TABLE IF NOT EXISTS messages(content TEXT NOT NULL,sendTime VARCHAR(13) NOT NULL, transactionId VARCHAR(20) NOT NULL, FOREIGN KEY(transactionId) REFERENCES trs(id) ON DELETE CASCADE)",
    "CREATE TABLE IF NOT EXISTS gratuities(address VARCHAR(21) NOT NULL, transactionId VARCHAR(20) NOT NULL, FOREIGN KEY(transactionId) REFERENCES trs(id) ON DELETE CASCADE)",
    // Indexes
    ...
    "CREATE INDEX IF NOT EXISTS gratuities_trs_id ON gratuities(transactionId)",
  ...
  async.eachSeries(
    ...
    function(err) {
      ...
      db.query("PRAGMA user_version", function(err, rows) {
        ...
        var nextVersions = Object.keys(migration)
          .sort()
          .filter(function(ver) {
