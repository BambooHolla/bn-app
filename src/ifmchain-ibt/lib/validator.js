...
ZSchema.registerFormat("gratuityAsset", function(obj) {
  if (
    obj &&
    obj.gratuity &&
    obj.gratuity.address &&
    obj.gratuity.gratuityTime
  ) {
...
ZSchema.registerFormat("dappAsset", function(obj) {
  if (
    ...
    obj.dapp &&
    obj.dapp.name &&
    obj.dapp.description &&
    obj.dapp.icon.length &&
    obj.dapp.coverPicture.length &&
    obj.dapp.developerAddress &&
    obj.dapp.downloadAddress.length &&
    obj.dapp.creatorPublicKey
  ) {
    return true;
  } else {
    return false;
...
ZSchema.registerFormat("multisignatureAsset", function(obj) {
  if (
    obj &&
    obj.multisignature &&
    obj.multisignature.min &&
    obj.multisignature.keysgroup &&
    obj.multisignature.lifetime
  ) {
    ...
    if (
      !Array.isArray(obj.multisignature.keysgroup) ||
      obj.multisignature.keysgroup.length < 1 ||
      obj.multisignature.keysgroup.length > 10
    ) {
      return false;
    }
    for (var i = 0; i < obj.multisignature.keysgroup.length; i++) {
      try {
        var publicKey = new Buffer(
          obj.multisignature.keysgroup[i].substring(1),
          "hex"
        );
        if (publicKey.length != 32) {
          return false;
...
ZSchema.registerFormat("destoryAsset", function(obj) {
  if (
    obj &&
    obj.destoryAsset &&
    obj.destoryAsset.abbreviation /*  && obj.destoryAsset.destoryAssets */
  ) {
    return true;
  } else {
    return false;
