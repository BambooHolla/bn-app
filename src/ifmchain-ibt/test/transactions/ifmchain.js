...
var secret =
  "group mobile series april amateur tattoo pen confirm still dream million outdoor believe ask end learn day useless supply border slogan luxury turkey honey";
var secret2 =
  "inject girl patient cry prefer depend goose piano picnic magnet seat cloth pistol blind excuse mixed proud light wheel jacket garlic teach fall bench";
//publickey come from secret
var publicKey =
  "cdfbd7588a91b5f4d531a12f940f9dd7cd46e256f7c4232036b9e7125b061d55";
var address = null;
...
describe("ifmchain.js", function() {
  it("create a password", function(done) {
    var password = new ifmchain.Mnemonic(
      256,
      ifmchain.Mnemonic.Words.ENGLISH
    ).toString();
  ...
  it("get user information by address", function(done) {
    ...
    address = ifmchain.addressCheck.generateBase58CheckAddress(
      keypair.publicKey
    );

    ifmchainApi.account.getUserByAddress(address).then(function(data) {
  ...
  it("get user information by username", function(done) {
    ...
    ifmchainApi.account.getUserByUsername(username).then(function(data) {
      data.success.should.equal(true);
      done();
    });
  });
  ...
  it("get block height should be ok", function(done) {
    ifmchainApi.block.getHeight().then(function(data) {
      data.success.should.equal(true);
  ...
  it("get block by id should be ok", function(done) {
    ifmchainApi.block.getBlockById(blockId).then(function(data) {
      data.success.should.equal(true);
      console.log(data);
      done();
    });
  ...
  it("get block by id should be ok", function(done) {
    ...
    ifmchainApi.block.getBlocks(info).then(function(data) {
      data.success.should.equal(true);
      console.log(data);
      done();
    });
  ...
  it("get confirmed number by id should be ok", function(done) {
    ifmchainApi.block
      .getComfirmedNumberByid(transactionId)
  ...
  it("get transactions by senderId and startHeight should be ok", function(done) {
    ...
    ifmchainApi.transaction.getTransactions(data).then(function(data) {
