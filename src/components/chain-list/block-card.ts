...
export class BlockCard extends PIXI.Graphics {
  ...
  delay_icon = new PIXI.Text("", this.style_delay_icon);
  ...
  drawLabels() {
    ...
    {
      delay_icon.text = label_config.delay_icon;
      delay_icon.x = right_base_line - getLabelWidth(delay_icon);
      delay_icon.y = H * 0.43;
    }
    {
  ...
  drawBlockModel(block: { numberOfTransactions: number | string; totalAmount: string; totalFee: string, delay?: boolean }) {
    this.setCacheAsBitmap(false);
    const {
      H,
      W,
    ...
    const right_base_line = W * 0.92;
    {
      // delay_icon.visible = !!block.delay;
      delay_icon.visible = true;
    }
    {
