...
export class SyncProgressSpinnerComponent extends AniBase {
  ...
  private _generateProgressSpinner(
    ...
    const ps = new ProgressSpinner(gradient_stops, arc_width, spinner_width);
...
export class ProgressSpinner extends PIXI.Container {
  constructor(
    public gradient_stops: Array<[number, string]> | number | [number, number],
  ...
  initDraw() {
    ...
    if (gradient_stops instanceof Array && gradient_stops[0] instanceof Array) {
      ...
      if (gradient_stops instanceof Array) {
        g.beginFill(gradient_stops[0] as number, gradient_stops[1] as number);
      } else {
