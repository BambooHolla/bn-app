...
import { ContactServiceProvider, ContactModel, ContactGroupList, ContactGroupItem } from "../../../providers/contact-service/contact-service";
...
export class AccountMyContactsPage extends SecondLevelPage {
  ...
  async loadMyContactList() {
    ...
    try {
      const { following: confirmed_contact_list, follower: unconfirm_contact_list } = await this.contactService.myContact.getPromise();
      // 绑定未确认的联系人
      if (this.isArrayDiff(this.unconfirm_contact_list, unconfirm_contact_list, contact_parser)) {
        this.unconfirm_contact_list = unconfirm_contact_list;
      }
      if (this.isArrayDiff(this.confirmed_contact_list, confirmed_contact_list, contact_parser)) {
        this.confirmed_contact_list = confirmed_contact_list;
        // 将已有的联系人进行分组
        const grouped_contact = this.contactService.contactGroup(confirmed_contact_list);
  ...
  @asyncCtrlGenerator.error("@@IGNORE_UNCONFIRM_CONTACT_ERROR")
  ...
  async addUnconfirmContact(contact: ContactModel) {
    ...
    return this._addUnconfirmContact(password, contact.address, pay_pwd, custom_fee);
  }
  @asyncCtrlGenerator.error()
  async removeConfirmedContact(contact: ContactModel) {
    ...
    return this._removeConfirmedContact(password, contact.address, pay_pwd, custom_fee);
  }
  @asyncCtrlGenerator.error("@@ADD_CONTACT_SUBMIT_ERROR")
  @asyncCtrlGenerator.loading("@@ADD_CONTACT_SUBMITING")
  @asyncCtrlGenerator.success("@@ADD_CONTACT_SUBMIT_SUCCESS")
  async _addUnconfirmContact(password: string, address: string, pay_pwd?: string, custom_fee?: number) {
    return this.contactService.addContact(password, address, pay_pwd, custom_fee);
  }
  @asyncCtrlGenerator.error("@@REMOVE_CONTACT_SUBMIT_ERROR")
  @asyncCtrlGenerator.loading("@@REMOVE_CONTACT_SUBMITING")
  @asyncCtrlGenerator.success("@@REMOVE_CONTACT_SUBMIT_SUCCESS")
  async _removeConfirmedContact(password: string, address: string, pay_pwd?: string, custom_fee?: number) {
    return this.contactService.addContact(password, address, pay_pwd, custom_fee, "-");
  }

  confirmToDelete(concat: ContactModel) {
    this.showConfirmDialog(this.getTranslateSync("CONFIRM_TO_REMOVE_CONTACT"), () => {
  ...
  @asyncCtrlGenerator.error("更新联系人列表失败，重试次数过多，已停止重试，请检测网络")
  ...
  async tryShowUserBalance(address: string) {
    ...
    await this.showSuccessDialog((parseFloat(account.balance) / 1e8).toFixed(8));
  }
}
