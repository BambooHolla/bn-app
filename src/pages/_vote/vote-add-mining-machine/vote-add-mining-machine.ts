...
export class VoteAddMiningMachinePage extends SecondLevelPage {
  ...
  private _getDelegateInfoAndFillForm(ip: string, port: number) {
    return this._getDelegateInfo(`http://${ip}:${port - 1}`).then(delegate => {
      if (delegate) {
  ...
  async getPeerInfo(peer: ExtendsSystemRuntime) {
    ...
    socket.on("systemStatus", ({ systemStatus: data }: { systemStatus }) => {
    ...
    try {
      await Promise.all([
        wait_io.promise,
        this.appFetch
          .get<any>(this.SYSTEM_RUNTIME.disposableServerUrl(`${host}:${port}`))
          .then(runtime => {
      ...
      if (
        ...
        await this.showErrorDialog(this.getTranslateSync("PEER_UNABLE"));
